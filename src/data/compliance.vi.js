export default {
    data: [
        {
            good_use: "Chính xác",
            poor_use: "Phê bình",
            best_approach: "phải nói như thế nào",
        },
        {
            good_use: "Tận tâm",
            poor_use: "Làm phiền",
            best_approach: "Liệt kê ưu, nhược điểm",
        },
        {
            good_use: "Nghiêm trọng",
            poor_use: "Đạo đức",
            best_approach: "Chính xác và logic",
        },
        {
            good_use: "Kiên định",
            poor_use: "Ngột ngạt",
            best_approach: "Cung cấp dẫn chứng",
        },
        {
            good_use: "Sắp đặt",
            poor_use: "Bướng",
            best_approach: "Giao thời hạn",
        },
        {
            good_use: "Cẩn thận",
            poor_use: "Thiếu quyết đoán",
            best_approach: "Cho thời gian",
        },
        {
            good_use: "Dè dặt",
            poor_use: "",
            best_approach: "Đừng vội vàng hay làm bất ngờ",
        },
    ],
    columns: [
        {
            field: "good_use",
            label: "Tốt",
        },
        {
            field: "poor_use",
            label: "Kém",
        },
        {
            field: "best_approach",
            label: "Cách tiếp cận tốt nhất",
        },
    ],
    intro: [
        "Quan tâm và đánh giá cao sự kiện dựa trên số liệu, dữ liệu, nguyên nhân",
        "Phân tích, hệ thống, có phương pháp",
        "Giải quyết công việc theo thứ tự và kế hoạch",
        "Thấu đáu và có tổ chức",
    ],
    features: [
        {
            icon: "🚀",
            label: "Nguồn động lực",
            value: "Quy trình chuẩn hóa.",
        },
        {
            icon: "😭",
            label: "Sợ",
            value: "Xung đột",
        },
        {
            icon: "🏆",
            label: "Giá trị cho tổ chức",
            value: "Chất lượng và tiêu chuẩn. Năng lực kỹ thuật",
        },
    ],
    labels: [
        "Tuân thủ / Nhường nhịn",
        "Có thể xem là quá thận trọng, quá nguyên tắc, một người làm mọi thứ theo nguyên tắc đề ra",
        "Đặc điểm nhận dạng",
        "Giao tiếp",
        "Nhìn chung, bạn là người",
        "Sử dụng cảm xúc",
    ],
    observable: [
        {
            label: "Tiếp cận",
            value:
                "Đúng giờ, luôn chuẩn bị và logic, thận trọng trong cách đặt câu hỏi.",
        },
        {
            label: "Sử dụng thời gian",
            value:
                "Phân tích theo dữ liệu trong quá khứ, xem xét tình hình hiện tại và đưa ra dự đoán. Xem thời gian như mỗi chuỗi các sự kiện nối tiếp, có nguyên nhân có kết quả",
        },
        {
            label: "Bắt tay",
            value: "Lỏng lẻo và ngắn gọn",
        },
        {
            label: "Bàn làm việc",
            value:
                "Rọn gàng, ngăn nắp, không cá nhân quá",
        },
        {
            label: "Trang phục",
            value:
                "Ăn mặc lịch sự, gọn gàng, thông minh, tránh những thứ lòe lẹt và luôn gọn gàng cả ngày",
        },
        {
            label: "Tác phong",
            value: "Rất lịch sự và đúng mực",
        },
        {
            label: "Giao tiếp bằng mắt",
            value:
                "Tránh giao tiếp bằng mắt, đặc biệt trong các tình huống tranh cãi. Thẳng tính khi chắc chắn về sự thật",
        },
        {
            label: "Ngôn ngữ cơ thể",
            value:
                "Có xu hướng thận trọng trong việc thể hiện cảm xúc qua cử chỉ hoặc nét mặt, có thể xem là không cá tính và lạnh lùng",
        },
        {
            label: "Xử lý bất đồng",
            value:
                "Tránh xung đột trừ khi có bằng chứng hỗ trợ",
        },
    ],
    communication: [
        {
            label: "Phong cách quản lý",
            value: "Bắt buộc theo nguyên tắc.",
        },
        {
            label: "Phong cách giao tiếp",
            value: "Bằng văn bản.",
        },
        {
            label: "Muốn biết",
            value:
                "Mọi thứ phải có số liệu, dữ kiện trình bài có trật tự, kiểm tra cẩn thận những gì đang thảo luận",
        },
        {
            label: "Văn phong",
            value:
                "Khá trang trọng, chính xác, dữ liệu được liệt kê rõ ràng.",
        },
    ],
    general: [
        "tư duy hệ thống và có tính logic cao",
        "rất cẩn thận và nguyên tắc, một kiểu người cầu toàn",
        "chính xác về giờ giấc",
        "đôi khi đắm chìm trong tiểu tiết",
        "không sẵn sàng hành động nếu chưa từng làm qua, đôi khi thiếu quyết đoán",
        "khi bị tấn công có xu hướng phòng thủ, né tránh các cuộc tranh cãi.",
        "không muốn chia sẽ cảm xúc.",
        "luôn lên kế hoạch cho mọi thứ rất rõ ràng",
        "đến đúng giờ và luôn đọc trước các tài liệu trước cuộc họp"
    ],
}